...
const paymentUploadInput = z.object({
  name: z.string().trim().min(1).max(255),
...
router.post("/storage/payment-proofs/request-url", requireAuth, async (req, res) => {
  const parsed = paymentUploadInput.safeParse(req.body);
  const user = (req as typeof req & { user: AuthUser }).user;
  if (!parsed.success) { res.status(400).json({ error: "Upload a PDF, JPG, PNG, or WEBP file up to 10 MB." }); return; }
  const [registration] = await db.select({ id: registrationsTable.id }).from(registrationsTable).where(eq(registrationsTable.userId, user.id)).limit(1);
  if (!registration) { res.status(404).json({ error: "Complete your BUICH registration first." }); return; }
  try {
    const { bucketName, prefix } = privateObjectParts();
    const objectName = `${prefix}/payment-proofs/${randomUUID()}`.replace(/^\/+/, "");
    const uploadURL = await signObjectUrl(bucketName, objectName, "PUT");
    res.json({ uploadURL, objectPath: `/objects/${objectName.replace(`${prefix}/`, "")}` });
  } catch (error) {
    req.log.error({ err: error }, "Error generating payment proof upload URL");
    res.status(500).json({ error: "Failed to prepare payment-proof upload." });
  }
});
